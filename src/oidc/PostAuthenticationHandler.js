...
module.exports = ({ docClient, globalConfiguration, cryptoKeys, globalHooksConfiguration }) => {
    ...
    api.get('/:connection_id', async (req, res) => {
        ...
        if(req.params.connection_id) {
            ...
            let opstate = req.cookies['opstate'] || queryObject.state;
            let requestParams = null;
            if(opstate != null) {
                let requestParamsSerial = utils.decryptPayload(opstate, cryptoKeys.stateEncryptionKey);
                if(requestParamsSerial){
                    requestParams = JSON.parse(requestParamsSerial);
                }
            }
            if(requestParams!=null) console.log("requestParams:", requestParams);
            
            let client_id = requestParams.client_id;
            console.log("requestParams.client_id:", requestParams.client_id);
            // validate client_id
            ...
            let issuer = utils.getCurrentOPHttpProtocol(req) + "://" + utils.getCurrentOPHost(req, globalConfiguration.wellKnownConfiguration.hosts_supported) + "/";
            let params = {
                request: req,
            ...
            if(queryObject.code) {
                ...
                try {
                    ...
                    console.log(response);
                     ...
                     return utils.sendErrorResponse(res, null, 'invalid_request', 'unable to retrieve identity from idp');
                }
                let id_token = (response && response.data) ? response.data.id_token : null;
                const decoded = jwt.decode(id_token, {complete: true});
                user =  decoded.payload;
                console.log(user);
            }else if(queryObject.id_token) {
                console.log("id_token=", queryObject.id_token);
                let id_token = queryObject.id_token;
                // always validate implicit flow id_token
                ...
                try {
                    ...
                    user = jwt.verify(id_token, signingKey, { algorithms: ['RS256']});
                    console.log(user);
                }
...
// execute post_authentication_script
          try {
...
// end of post authentication script            
                        
        }else{
            return utils.sendErrorResponse(res, null, 'invalid_request', 'connection_id missing');
        }
    });
